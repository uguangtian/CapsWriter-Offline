...
async def on_hotkey_pressed():
    try:
        ...
        # 打开 everything
        await asyncio.create_subprocess_exec(Config.everything_exe_path, '-search', text)
...
async def search_selected_text_with_everything():
    try:
        ...
        # 注册热键
        keyboard.add_hotkey(Config.search_selected_text_with_everything_shortcut, hotkey_callback)
        
        await asyncio.Event().wait()  # 阻塞当前协程，直到被取消或热键被触发
    except Exception as e:
        console.print(e)
    finally:
        keyboard.remove_hotkey(Config.search_selected_text_with_everything_shortcut)  # 移除热键
