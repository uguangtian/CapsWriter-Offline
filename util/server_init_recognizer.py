...
def init_recognizer(queue_in: Queue, queue_out: Queue, sockets_id, config: dict):
    # 提取配置
    server_config = config.get("server", {})
    model_name = server_config.get("model")
    format_punc = server_config.get("format_punc")
    ...
    try:
        if model_name == "Paraformer":
            # 打印模型路径信息以便调试
            paraformer_args = config.get("paraformer_args", {})
        ...
        if model_name == "Paraformer":
            paraformer_args = config.get("paraformer_args", {})
            model_path = paraformer_args.get("paraformer")
    ...
    while True:
        ...
        if model_name == "Paraformer":
            ...
            result = recognize(recognizer, task, config=config)  # 执行识别
        else:
            console.print(f"[DEBUG] 使用 {model_name} 模型处理音频", style="cyan")
            result = recognize(recognizer, task, config=config)  # 执行识别
