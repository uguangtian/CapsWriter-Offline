...
async def call_claude_api(text: str, action: str = "polish", **kwargs) -> str:
    ...
    # 获取额外参数
    temperature = kwargs.get('temperature', ClaudeConfig.temperature)
    max_tokens = kwargs.get('max_tokens', ClaudeConfig.max_tokens)
    model = kwargs.get('model', ClaudeConfig.model)
...
# WebSocket服务器实现
async def claude_server(websocket):
    """处理WebSocket连接和请求
    ...
    try:
        async for message in websocket:
            try:
                ...
                if save_to_file:
                    ...
                    polish_service = TextPolishService(model_type='claude')
        ...
        print(f"[Claude] WebSocket连接异常: {str(e)}")
    finally:
        print(f"[Claude] WebSocket连接已关闭")


def run_claude_service(host="127.0.0.1", port=None):
    """启动Claude服务
    ...
    if port is None:
        port = ClaudeConfig.claude_port if hasattr(ClaudeConfig, 'claude_port') else 6020
    
    start_server = websockets.serve(
        claude_server, host, port
    )
    print(f"[Claude] 正在启动服务，监听地址: {host}:{port}")
